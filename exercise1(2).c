...
int main() 
{
    ...
    for (int k = 0; k < num_n; k++) 
    {
        ...
        {
            xs[i] = -1 + 2.0 * i / n;
            ys[i] = f1(xs[i]);       //从-1开始，每步增加2/n，计算节点对应的函数值
        }
        ...
        if (n == 20) 
        {
            ...
            fclose(fp);
            printf("已生成 f(x) = sqrt(1 - x²) (n=20) 的绘图数据到 f1_plot_data.txt\n");
        }
    ...
    for (int k = 0; k < num_n; k++) 
    {
        ...
        if (n == 20) 
        {
            ...
            fclose(fp);
            printf("已生成 f(x) = 1 / (1 + 25x²) (n=20) 的绘图数据到 f2_plot_data.txt\n");
        }
